...
end_button = ttk.Button(root, text="End Session", command=lambda: [
    open_popup2(), stop()])
...
#
# def animate(i):
#     data = np.hstack((times[:i, np.newaxis], beat[:i, np.newaxis]))
#     sctr.set_offsets(data)
#     return sctr,

# ani = FuncAnimation(fig, animate, 68, interval=5,
#                              blit=True)
# root.update()
